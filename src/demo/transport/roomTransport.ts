...
import { ReconnectTimeMode, StompWebSocketClient, WindowWebSocketClient } from "../../lib";

/** 데모가 지원하는 프로토콜. 화면에서 전환하며 같은 시나리오를 확인한다. */
export type Protocol = "stomp" | "window";
...
export function createRoomTransport(protocol: Protocol, room: string): RoomTransport {
  return protocol === "stomp" ? stompTransport(room) : windowTransport(room);
}

/** 방 이름이 실제로 어떤 주소가 되는지. 화면 표시와 전송이 같은 계산을 쓰도록 여기 한 곳에 둔다. */
export function roomAddress(protocol: Protocol, room: string): string {
  return protocol === "stomp" ? `/topic/${room}` : `${echoURL}/?room=${encodeURIComponent(room)}`;
}
